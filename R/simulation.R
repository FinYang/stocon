...
#' @export
#'
sim_simple <- function(R1=NULL, mu=NULL, vol=NULL, Tn=10, N=5, M=1e4, defaultsetting = TRUE){
  if(length(R1 & mu & vol)==0){
    if(defaultsetting) N <- 5
    R1 <- mu <- vol <- NULL
  ...
  # return at time 0
  if(length(R1 & mu & vol) == 0 & defaultsetting){
  ...
  } else if(length(R1 & mu & vol) == 0 & !defaultsetting){
    Rt[[1]] <- matrix(runif(M*N, min = 0.01, max = 0.14), ncol = N)
    mu <- runif(N, min = 0.0001, max = 0.002)
    vol <- runif(N, min = 0.001, max = 0.1)
  } else {
    Rt[[1]] <- R1
  }
  # covariance matrix
  rho_do <- function(i,j, par=0.3){
    exp(-par*(i-j)) %>% return()
  }
