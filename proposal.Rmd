...
Consider the typical high dimensional portfolio selection problem in terms of random process, we can get

$$V_0(x_0)=\sup_{\mathbf{c}_0, \mathbf{\beta}_0}\mathbf{E}[u(C_0(W_0, c_0))+\sum_{j=1}^{T-1}u(C_j({W}_j(\mathbf\beta_{j-1}, \mathbf{r}_{j-1}),c_j))\delta^{j} + u(W_T(\mathbf\beta_{T-1}, \mathbf{r}_{T-1}))\delta^T]$$

contional on the boundary condition 

$$V_T(x_T)=\sup_{\mathbf{\beta}_{T-1}}\mathbf{E}[ u(W_T(\mathbf\beta_{T-1}, \mathbf{r}_{T-1}))]$$

where

* $\mathbf{r}_t$ is a vector of returns of different assets at time $t$.
* $\mathbf\beta_t$ is a vector of weight of different assets in this portfolio.
...
$$\begin{aligned}W_{t+1}&=(W_t-C_t)(\mathbf{\beta}_t\cdot \mathbf{r}_t+1)\\ 
C_t&=W_t\cdot \frac{\exp\{c_{1, t}+c_{2,t}W_{t}\}}{1+\exp\{c_{1, t}+c_{2,t}W_{t}\}}\end{aligned}$$
...
$$V_t(x_t)=\sup_{\mathbf{c}_t, \mathbf{\beta}_t}\mathbf{E}[\sum_{j=t}^{T-1}u(C_j({W}_j(\mathbf\beta_{j-1}, \mathbf{r}_{j-1}),c_j))\delta^{j-t} + u(W_T(\mathbf\beta_{T-1}, \mathbf{r}_{T-1}))\delta^{T-t}]$$
