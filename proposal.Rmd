...
$$V_0(x_0)=\sup_{\mathbf{c}_0, \mathbf{\beta}_0}\mathbf{E}[u(C_0(W_0, c_0))+\sum_{t=1}^{T-1}u(C_t({W}_t(\mathbf\beta_{t-1}, \mathbf{r}_{t-1}),c_t))\delta^{t} + u(W_T(\mathbf\beta_{T-1}, \mathbf{r}_{T-1}))\delta^T]$$
where
...
* $c_t$ is the control variable (or a vector of the control variables) which in turn determines consumption $C_t$.
* $u$ is the utility function depending on consumption or the terminal wealth.

The state of $W_t$


$$C_t=W_t\times\frac{\exp{}}{}$$
