...
$$\begin{aligned}f_t(W_t) &= \min_{\{C_s\}^T_{s=t}, \{\beta_s\}^{T-1}_{s=t}}E[\sum^{T}_{s=t+1}\delta^{s-t} \cdot(C^2_{s} - 2\lambda C_{s})+\delta^{T-t}\cdot (W_T^2 - 2\lambda W_T)]\\ &=\min_{\{C_s\}^T_{s=t}, \{\beta_s\}^{T-1}_{s=t}}E[\sum^{T}_{s=t+1}\delta^{s-t} \cdot((C_s-\lambda)^2-\lambda^2)+\delta^{T-t}\cdot ((W_T-\lambda)^2 -\lambda^2)]\end{aligned}$$
